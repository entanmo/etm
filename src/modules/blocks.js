...
__private._selfPendingPropose = false;

__private._loadBlocksLimit = 200;

const _loadBlockTimeout = () => {
  let limit = __private._loadBlocksLimit;
...
const _loadBlockElapse = (elapse) => {
  const kMaxElapse = 1500;      // millisecond
  const kIncStep = 10;          // increment 10 per time
  const kMaxLimit = 200;        // max limit 200
  if (elapse <= kMaxElapse) {
    // increment
    let limit = __private._loadBlocksLimit;
    limit += kIncStep;
...
__private.getByField = function (field, cb) {
...
__private.popLastBlock = function (oldLastBlock, callback) {
...
Blocks.prototype.getCommonBlock = function (peer, height, cb) {
  ...
  async.whilst(
    ...
    function (next) {
      count++;
      __private.getIdSequence2(lastBlockHeight, function (err, data) {
        if (err) {
          return next(err)
        }
        var params = {
...
Blocks.prototype.loadBlocksOffset = function (limit, offset, verify, cb) {
  ...
  library.dbSequence.add(function (cb) {
    library.dbLite.query(FULL_BLOCK_QUERY +
      ...
      "ORDER BY b.height, t.rowid" +
      "", params, __private.blocksDataFields, function (err, rows) {
        // Notes:
        // If while loading we encounter an error, for example, an invalid signature on a block & transaction, then we need to stop loading and remove all blocks after the last good block. We also need to process all transactions within the block.
        if (err) {
        ...
        async.eachSeries(blocks, function (block, next) {
          ...
          if (verify) {
            if (!__private.lastBlock || !__private.lastBlock.id) {
              // apply genesis block
...
Blocks.prototype.setLastBlock = function (block) {
  ...
  if (global.Config.netVersion === 'mainnet') {
    global.featureSwitch.enableLongId = __private.lastBlock.height >= 1700000
    global.featureSwitch.enable1_3_0 = __private.lastBlock.height >= 2920000
    global.featureSwitch.enableClubBonus = __private.lastBlock.height >= 3320000
    global.featureSwitch.enableMoreLockTypes = global.featureSwitch.enableClubBonus
    global.featureSwitch.enableLockReset = __private.lastBlock.height >= 4290000
  } else {
  ...
  if (global.Config.netVersion === 'mainnet' && __private.lastBlock.height < 1854000) {
...
Blocks.prototype.verifyBlock = function (block, votes, cb) {
  ...
  var expectedReward = __private.blockStatus.calcReward(block.height);
...
Blocks.prototype.loadBlocksFromPeer = function (peer, lastCommonBlockId, cb) {
  ...
  async.whilst(
    ...
    function (next) {
      count++;
      //loadblocks
      const limit = __private._loadBlocksLimit;
      const params = {
        limit,
      ...
      modules.peer.request('loadblocks', params, peer, (err, body) => {
        ...
        // dynamic change the limit per valid elapse.
        const elapse = Math.floor((endElapse - startElapse) * 1000);
        _loadBlockElapse(elapse);
...
Blocks.prototype.generateBlock = function (keypair, timestamp, cb) {
  ...
  var ready = [];
  if (__private._selfPendingPropose || library.base.consensus.hasPendingBlock(timestamp)) {
    return setImmediate(cb);
  }
    ...
    async.waterfall([
      ...
      function (activeKeypairs, next) {
        ...
        if (library.base.consensus.hasEnoughVotes(localVotes, block.timestamp - self.getLastBlock().timestamp)) {
          ...
          __private._selfPendingPropose = true;
...
Blocks.prototype.onReceiveBlock = function (block, votes) {
  if (modules.loader.syncing() || !__private.loaded) {
    return;
  }

  if (__private.blockCache[block.id]) {
    return;
  }
  ...
  library.sequence.add(function receiveBlock(cb) {
    ...
    if (block.previousBlock == __private.lastBlock.id && __private.lastBlock.height + 1 == block.height) {
      ...
      cb("Fork");
    } else if (block.height > __private.lastBlock.height + 1) {
...
Blocks.prototype.onReceivePropose = function (propose) {
  ...
  if (__private.proposeCache[propose.hash]) {
    return;
  }
  __private.proposeCache[propose.hash] = true;
  ...
  library.sequence.add(function receivePropose(cb) {
    ...
    if (propose.height != __private.lastBlock.height + 1) {
...
Blocks.prototype.getSupply = function () {
  let height = __private.lastBlock.height
  return __private.blockStatus.calcSupply(height)
}
  ...
  library.scheme.validate(query, /*{
    ...
    properties: {
      ...
      library.dbSequence.add(function (cb) {
        ...
        __private.getByField(field, function (err, block) {
          if (!block || err) {
...
shared.getFullBlock = function (req, cb) {
  ...
  library.scheme.validate(query, /*{
    ...
    properties: {
  ...
  }*/ scheme.getFullBlock, function (err) {
      if (err) {
        return cb(err[0].message);
      }
      ...
      library.dbSequence.add(function (cb) {
        ...
        library.dbLite.query(FULL_BLOCK_QUERY + condition, {}, __private.blocksDataFields, function (err, rows) {
...
shared.getMilestone = function (req, cb) {
  ...
  cb(null, { milestone: __private.blockStatus.calcMilestone(height) });
}
...
shared.getReward = function (req, cb) {
  ...
  var query = req.body, height = __private.lastBlock.height;
  cb(null, { reward: __private.blockStatus.calcReward(height) });
}
...
shared.getSupply = function (req, cb) {
  ...
  var query = req.body, height = __private.lastBlock.height;
  cb(null, { supply: __private.blockStatus.calcSupply(height) });
}
...
shared.getStatus = function (req, cb) {
  ...
  cb(null, {
    ...
    milestone: __private.blockStatus.calcMilestone(height),
    reward: __private.blockStatus.calcReward(height),
    supply: __private.blockStatus.calcSupply(height)
  });
}

...
var Sandbox = require('../utils/sandbox');
...
__private.attachApi = function () {
  ...
  router.post('/uninstall', function (req, res, next) {
    req.sanitize(req.body, {
      ...
      __private.get(body.id, function (err, dapp) {
        ...
        __private.removing[body.id] = true;
          ...
          __private.stop(dapp, function (err) {
            if (err) {
              ...
              __private.removeDApp(dapp, function (err) {
                __private.removing[body.id] = false;
  ...
  router.get('/removing', function (req, res, next) {
    var ids = [];
    for (var i in __private.removing) {
      if (__private.removing[i]) {
        ids.push(i);
      }
    }
...
__private.getDAppBalance = function (req, cb) {
...
__private.getDAppBalances = function (req, cb) {
  if (!req.params) return cb('Invalid parameters')
  if (!req.params.dappId || req.params.dappId.length > 64) return cb('Invalid dapp id')
  let filter = {
...
__private.createBasePathes = function (cb) {
  async.series([
    ...
    function (cb) {
      var dappsPublic = path.join(__private.appPath, 'public', 'dist', 'dapps');
...
__private.removeDApp = function (dapp, cb) {
  ...
  async.waterfall([
    ...
    async.apply(__private.readJson, path.join(dappPath, "blockchain.json")),
...
__private.symlink = function (dapp, cb) {
  ...
  var dappPublicLink = path.resolve(__private.appPath, "public", "dist", "dapps", dapp.transactionId);
...
__private.dappRoutes = function (dapp, cb) {
  ...
  if (!__private.defaultRouteId) {
    __private.defaultRouteId = dapp.transactionId;
    library.network.app.use('/api/dapps/default/', __private.routes[dapp.transactionId]);
  }
  library.network.app.use('/api/dapps/' + dapp.transactionId + '/', __private.routes[dapp.transactionId]);
  library.network.app.use(function (err, req, res, next) {
    if (!err) return next();
    library.logger.error(req.url, err.toString());
    res.status(500).send({ success: false, error: err.toString() });
...
__private.launchApp = function (dapp, params, cb) {
  ...
  __private.readJson(path.join(dappPath, "config.json"), function (err, dappConfig) {
    if (err) {
    ...
    async.eachSeries(dappConfig.peers, function (peer, cb) {
      ...
      var sandbox = new Sandbox(dappPath, dapp.transactionId, params, __private.apiHandler, true, library.logger);
...
DApps.prototype.onBlockchainReady = function () {
  __private.getInstalledIds(function (err, dappIds) {
    if (err) {
    ...
    async.eachSeries(dappIds, function (id, next) {
      ...
      library.model.getDAppByName(WITNESS_CLUB_DAPP_NAME, function (err, clubInfo) {
        if (err) {
          library.logger.error('Failed to query entanmo witness club', err)
        } else if (!clubInfo) {
          library.logger.warn('EnTanMo witness club dapp is not found')
        } else {
...
shared.registerInterface = function (req, cb) {
  ...
  __private.routes[dappId][method](path, function (req, res) {
    var reqParams = {
